...
1 2 3 1 4 5 2 3 6
3
Output : Find The Max and Min in Every 3 length Subarray 
  = > 3 3 4 5 5 5 6
The Idea : Sliding Window with Multiset  
...
void solve()
{
    ...
    for(int i = 0 ; i<n-k+1 ; i++)
    {
        ...
        mst.erase(mst.find(arr[i])) ; // Erase The One Occ of arr[i] ;
        if(i+k<n) mst.insert(arr[i+k]) ;  // insert next index  
    }

    // We can use set if we Store set<pair<value , index>> st ; 
    // st.erase({arr[i] , i }) ; 
