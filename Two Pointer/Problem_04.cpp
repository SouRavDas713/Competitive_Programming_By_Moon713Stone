...
Output: false
Input: name = "saeede", typed = "ssaaeeddddeeeeefee"
Output: false 

The Idea :
Match One By One ... And Check Many Edge Case ... 
if(i==j) i++ ; j++ ; 
else check i-1 == j  ; yes ? j++ ; else false  ; 
...
void solve()
{
    ...
    {
        // aab || aaabb
        if(i>=name.ln() && j>=type.ln())
        {
            ans = 1 ; 
            break ; 
        }
        // aabb || aaaabbbbbbbbb [ Match the Last Char ]
        if(i>=name.ln())
        {
            if(type[j]==name.back())
            {
