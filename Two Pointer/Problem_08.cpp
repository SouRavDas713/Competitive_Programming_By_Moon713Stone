...
The Idea : Two Pointer ... which Pointer Small ? update it ... 

Another Easy Problem : https://codeforces.com/contest/1669/problem/F

Another Easy Problem : https://leetcode.com/problems/maximum-matching-of-players-with-trainers/
...
void solve()
{
    ...
    while(i<j)
    {
        ll current  = min(arr[i] , arr[j]) * (j-i) ; 
        ans = max(ans , current) ; 
        if(arr[i]<arr[j]) i++ ; 
        else j-- ; 
    }
